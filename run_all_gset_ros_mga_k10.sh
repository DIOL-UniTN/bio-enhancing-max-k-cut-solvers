...
echo "========================================" | tee -a "$LOG_FILE"
echo "Inizio esecuzione batch Gset - ros_mga k=10" | tee -a "$LOG_FILE"
...
for GSET_NUM in "${GSET_NUMBERS[@]}"; do
    ...
    (cd ROS && python main.py --alg ros_mga --graph_type gset --gset "$GSET_NUM" --weight_mode 1 --k 10) > "$OUTPUT_FILE" 2>&1
    EXIT_CODE=$?
