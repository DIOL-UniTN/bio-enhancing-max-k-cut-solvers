...
echo "========================================" | tee -a "$LOG_FILE"
echo "Inizio esecuzione batch Gset - ros_aco k=2" | tee -a "$LOG_FILE"
...
for GSET_NUM in "${GSET_NUMBERS[@]}"; do
    ...
    (cd ROS && python main.py --alg ros_aco --graph_type gset --gset "$GSET_NUM" --weight_mode 1 --k 2) > "$OUTPUT_FILE" 2>&1
    EXIT_CODE=$?
